  ...
  const content = (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <DiffViewer title={t("oldValue")} data={log.oldValue} />
        <DiffViewer title={t("newValue")} data={log.newValue} />
      </div>

      <DiffViewer title={t("changesDetected")} data={log.changes} />
    </div>
  );

  const title = `${t("logDetails")}: ${log.id.substring(0, 8)}`;
