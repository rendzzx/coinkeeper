...
function AutoLockCard() {
  ...
  const handleTimeoutChange = (value: string) => {
    if (settings.user) {
      ...
      updateSettings({user: {...settings.user, autoLockTimeout: newTimeout}});
    }
  };

  if (!settings.user?.passwordHash) return null;
    ...
    <Card>
      ...
      <CardContent>
        <div className="w-full max-w-sm">
          <Select
            value={String(settings.user.autoLockTimeout ?? 0)}
...
export function PasswordManager() {
  ...
  const onSubmit = async (data: PasswordFormValues) => {
    ...
    const updatedUser = {
      ...
      autoLockTimeout: isPasswordSet ? settings.user.autoLockTimeout : 0,
    };

    await updateSettings({user: updatedUser});
  ...
  const handleRemovePassword = async (data: RemovePasswordFormValues) => {
    if (!settings.user || !settings.user.passwordHash) return;
  ...
  return (
    <div
      className={cn(
        'grid gap-6',
        isPasswordSet ? 'md:grid-cols-2' : 'grid-cols-1'
      )}
    >
      <Card>
        <CardHeader>
