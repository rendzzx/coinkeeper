      ...
      switch (action.type) {
        case "SET_STATE":
          await db.transaction("rw", db.tables, async () => {
            await Promise.all(
              Object.keys(initialMockState).map((key) => {
          ...
          await logHistory({
            action: "system",
            entity: "other",
            description: "Application state reset or imported.",
          });
          break;
        ...
        case "ADD_BUDGET":
          ...
          await logHistory({
            ...
            description: `Budget "${action.payload.name}" created.`,
          });
        ...
        case "UPDATE_BUDGET": {
          ...
          await logHistory({
            ...
            oldValue: oldBudget,
            newValue: action.payload,
            changes: diffObjects(oldBudget!, action.payload),
            description: `Budget "${action.payload.name}" updated.`,
          });
        ...
        case "DELETE_BUDGET": {
          ...
          if (oldBudget) {
            ...
            await db.restoreBin.add({
              id: restoreId,
              entity: "budget",
              entityId: oldBudget.id,
              payload: oldBudget,
            ...
            await logHistory({
              ...
              status: "pending",
              restoreId,
              description: `Budget "${oldBudget.name}" deleted.`,
            });
            toast({
              title: t("toastBudgetDeleted"),
              description: t("toastBudgetDeletedDesc", {
                budgetName: oldBudget.name,
              }),
            });
          }
          break;
        ...
        case "ADD_CATEGORY":
          ...
          await logHistory({
            ...
            entityId: action.payload.id,
            newValue: action.payload,
            description: `Category "${action.payload.name}" created.`,
          });
        ...
        case "UPDATE_CATEGORY": {
          ...
          await logHistory({
            ...
            newValue: action.payload,
            changes: diffObjects(oldCategory!, action.payload),
            description: `Category "${action.payload.name}" updated.`,
          });
        ...
        case "DELETE_CATEGORY": {
          ...
          if (oldCategory) {
            ...
            await logHistory({
              ...
              status: "pending",
              restoreId,
              description: `Category "${oldCategory.name}" deleted.`,
            });
        ...
        case "ADD_SUBCATEGORY": {
          ...
          if (parent) {
            const newValue = {
              subcategories: [...parent.subcategories, subCategory],
            };
            await db.categories.update(parentCategoryId, newValue);
            await logHistory({
              action: "update",
              entity: "category",
              entityId: parentCategoryId,
              description: `Subcategory "${subCategory.name}" added to "${parent.name}".`,
        ...
        case "UPDATE_SUBCATEGORY": {
          ...
          const parent = await db.categories.get(parentCategoryId);
          if (parent) {
            const oldSubCategory = parent.subcategories.find(
              (sc) => sc.id === subCategory.id
            );
            const updatedSubcategories = parent.subcategories.map((sc) =>
              sc.id === subCategory.id ? subCategory : sc
            );
            await db.categories.update(parentCategoryId, {
            ...
            await logHistory({
              ...
              newValue: subCategory,
              changes: diffObjects(oldSubCategory!, subCategory),
          ...
          if (parent) {
            ...
            if (oldSubCategory) {
              ...
              await db.restoreBin.add({
                ...
                payload: {...oldSubCategory, parentCategoryId},
                deletedAt: new Date().toISOString(),
              });
              const updatedSubcategories = parent.subcategories.filter(
                (sc) => sc.id !== subCategoryId
              );
              await db.categories.update(parentCategoryId, {
                subcategories: updatedSubcategories,
              ...
              await logHistory({
                ...
                status: "pending",
                restoreId,
                description: `Subcategory "${oldSubCategory.name}" deleted.`,
              });
              toast({
                title: t("toastSubcategoryDeleted"),
                variant: "destructive",
              });
            }
          }
        ...
        case "ADD_TAG":
          ...
          await logHistory({
            ...
            entityId: action.payload.id,
            newValue: action.payload,
            description: `Tag "${action.payload.name}" created.`,
          });
        ...
        case "UPDATE_TAG": {
          ...
          await logHistory({
            ...
            oldValue: oldTag,
            newValue: action.payload,
            changes: diffObjects(oldTag!, action.payload),
            description: `Tag "${action.payload.name}" updated.`,
          });
        ...
        case "DELETE_TAG": {
          ...
          if (oldTag) {
            ...
            await logHistory({
              ...
              status: "pending",
              restoreId,
              description: `Tag "${oldTag.name}" deleted.`,
            });
        ...
        case "ADD_DEBT": {
          ...
          if (debtPayload.sourceTransactionId) {
            ...
            const newCategory =
              debtPayload.type === "debt" ? "sub-sys-debt-2" : "sub-sys-debt-1";
            await db.transactions.update(debtPayload.sourceTransactionId, {
              debtId: debtPayload.id,
              categoryId: newCategory,
            });
            await logHistory({
              action: "create",
              entity: "debt",
              entityId: debtPayload.id,
              newValue: debtPayload,
              description: `Debt for "${debtPayload.person}" created from existing transaction.`,
            });
          } else {
            const transactionType =
              debtPayload.type === "debt" ? "income" : "expense";
            const categoryId =
              debtPayload.type === "debt" ? "sub-sys-debt-2" : "sub-sys-debt-1";
            ...
            const newTransaction: Transaction = {
              ...
              amount: debtPayload.initialAmount,
              walletId: debtPayload.walletId!,
              categoryId: categoryId,
              date: debtPayload.startDate,
              notes:
                debtPayload.notes ||
                `Initial amount for ${debtPayload.type} with ${debtPayload.person}`,
              tags: debtPayload.tags || [],
              attachments: debtPayload.attachments,
              debtId: debtPayload.id,
            };
            const updatedDebt = {
              ...debtPayload,
            ...
            if (wallet) {
              const newBalance =
                transactionType === "income"
                  ? wallet.balance + debtPayload.initialAmount
                  : wallet.balance - debtPayload.initialAmount;
            ...
            await logHistory({
              ...
              description: `Debt for "${updatedDebt.person}" created.`,
            });
            await logHistory({
              action: "create",
              entity: "transaction",
              ...
              description: `Source transaction for debt created.`,
        ...
        case "UPDATE_DEBT": {
          ...
          await logHistory({
            ...
            oldValue: oldDebt,
            newValue: action.payload,
            changes: diffObjects(oldDebt!, action.payload),
            description: `Debt for "${action.payload.person}" updated.`,
          });
        ...
        case "DELETE_DEBT": {
          ...
          if (oldDebt) {
            ...
            await logHistory({
              ...
              status: "pending",
              restoreId,
              description: `Debt for "${oldDebt.person}" deleted.`,
            });
            toast({
              title: t("toastDebtDeleted"),
              description: t("toastDebtDeletedDesc", {person: oldDebt.person}),
            });
          }
          break;
        }
        ...
        case "ADD_DEBT_PAYMENT": {
          ...
          const newStatus =
            newPaidAmount >= debtToUpdate.initialAmount ? "paid" : "active";
          ...
          await logHistory({
            ...
            description: `Payment of ${transaction.amount} for debt "${debtToUpdate.person}".`,
            context: {
              source: "manual",
          ...
          if (newStatus === "paid") {
            await logHistory({
              ...
              entity: "debt",
              entityId: debtId,
              description: `Debt for "${debtToUpdate.person}" marked as paid.`,
            });
          }
          break;
        }
        case "RESTORE_FROM_BIN": {
          ...
          switch (binItem.entity) {
            ...
            case "transaction":
              ...
              if (wallet) {
                const newBalance =
                  binItem.payload.type === "income"
                    ? wallet.balance + binItem.payload.amount
                    : wallet.balance - binItem.payload.amount;
                await db.wallets.update(wallet.id, {balance: newBalance});
              }
              if (binItem.payload.transferId) {
                const otherSideBinItem = await db.restoreBin
                  .where({
                    entity: "transaction",
                    entityId: binItem.payload.transferId,
                  })
                  .first();
                if (otherSideBinItem) {
                  await db.transactions.add(otherSideBinItem.payload);
                  await db.restoreBin.delete(otherSideBinItem.id);
                  const otherHistoryLog = await db.history
                    .where({restoreId: otherSideBinItem.id})
                    .first();
                  if (otherHistoryLog)
                    await db.history.update(otherHistoryLog.id, {
                      status: "success",
                    });
                }
              }
            ...
            case "category":
              if (binItem.payload.parentCategoryId) {
                const parent = await db.categories.get(
                  binItem.payload.parentCategoryId
                );
                if (parent) {
          ...
          const historyLog = await db.history
            .where({restoreId: binItem.id})
            .first();
          if (historyLog) {
            await db.history.update(historyLog.id, {
              status: "success",
              restoreId: null,
            });
          }
          ...
          await logHistory({
            action: "restore",
            entity: binItem.entity,
            entityId: binItem.entityId,
            newValue: binItem.payload,
            description: `Restored ${binItem.entity} "${
              binItem.payload.name || binItem.entityId
            }".`,
          });
          toast({
            title: t("toastItemRestored"),
            description: t("toastItemRestoredDesc"),
          });
          break;
        }
        case "PERMANENT_DELETE_FROM_BIN": {
          ...
          await logHistory({
            ...
            entity: binItem.entity,
            entityId: binItem.entityId,
            oldValue: binItem.payload,
            status: "success",
            description: `Permanently deleted ${binItem.entity} "${
              binItem.payload.name || binItem.entityId
            }".`,
          });
          toast({
            title: t("toastItemPermanentlyDeleted"),
            description: t("toastItemPermanentlyDeletedDesc"),
            variant: "destructive",
          });
          break;
        }
...
export function AppProvider({
  ...
  const [handleLock, setHandleLock] = useState<() => void>(() => () => {});
  const [handleExport, setHandleExport] = useState<() => void>(() => () => {});
  const [handleImport, setHandleImport] = useState<() => void>(() => () => {});
