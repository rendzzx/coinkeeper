...
function getInitialCache(devMode: boolean): AppSettings | null {
  try {
    ...
    if (parsed.version === CACHE_VERSION) {
      if (devMode) {
        console.info("%c[Cache] Loaded settings from sessionStorage", "color:#3b82f6;font-weight:bold;");
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  const [cachedState, setCachedState] = useState<AppSettings | null>(() => getInitialCache(devModeRef.current));
      ...
      debounceTimer.current = setTimeout(() => {
        try {
          ...
          if (devModeRef.current) {
            console.info(`%c[Cache] Settings cache updated at ${new Date().toLocaleTimeString()}`, "color:#f59e0b;font-weight:bold;");
          }
        } catch (err) {
  ...
  const updateSettings = useCallback(async (newSettings: Partial<AppSettings>) => {
    await db.transaction('rw', db.settings, db.history, async () => {
        ...
        const currentSettings = currentSettingsEntry?.value || initialMockState.settings;
        ...
        await logHistory({ action: 'update', entity: 'settings', entityId: 'userSettings', oldValue: currentSettings, newValue: updatedSettings, changes: diffObjects(currentSettings, newSettings), description: 'Application settings updated.' });
