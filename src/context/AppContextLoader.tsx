...
import type { AppState, Category, DashboardCardKey, Transaction } from '@/lib/types';
...
export const initialMockState: AppState = {
  ...
  settings: {
    ...
    dashboardCardOrder: (mockSettings.dashboardCardOrder as DashboardCardKey[]) || ALL_DASHBOARD_CARDS.map(c => c.id),
...
async function initializeDatabase() {
  ...
  if (settingsCount === 0) {
    console.log("%c[DB] Populating with mock data...", "color:#22c55e;font-weight:bold;");
    ...
    await db.transaction("rw", db.tables, async () => {
      ...
      await db.scheduledTransactions.bulkAdd(initialMockState.scheduledTransactions);
...
function getInitialCache(devMode: boolean): AppState | null {
  try {
    ...
    if (parsed.version === CACHE_VERSION) {
      if (devMode) {
        console.info("%c[Cache] Loaded main context from sessionStorage", "color:#3b82f6;font-weight:bold;");
        ...
        if (liveState) {
            ...
            debounceTimer.current = setTimeout(() => {
                try {
                    ...
                    if (devMode) {
                        console.info(`%c[Cache] Main context session cache updated at ${new Date().toLocaleTimeString()}`, "color:#f59e0b;font-weight:bold;");
                    }
                } catch (err) {
...
function AppProviderWithLiveQueries({ children }: { children: ReactNode }) {
  ...
  const [cachedState] = useState<AppState | null>(() => getInitialCache(devMode));

  // Fetch all data centrally
  const wallets = useLiveQuery(() => db.wallets.toArray(), [], cachedState?.wallets);
  const walletTypes = useLiveQuery(() => db.walletTypes.toArray(), [], cachedState?.walletTypes);
  const transactions = useLiveQuery(() => db.transactions.orderBy('date').reverse().toArray(), [], cachedState?.transactions);
  const scheduledTransactions = useLiveQuery(() => db.scheduledTransactions.toArray(), [], cachedState?.scheduledTransactions);
  const categories = useLiveQuery(() => db.categories.toArray(), [], cachedState?.categories);
  const tags = useLiveQuery(() => db.tags.toArray(), [], cachedState?.tags);
  const budgets = useLiveQuery(() => db.budgets.toArray(), [], cachedState?.budgets);
  const debts = useLiveQuery(() => db.debts.toArray(), [], cachedState?.debts);
  const history = useLiveQuery(() => db.history.orderBy('timestamp').reverse().toArray(), [], cachedState?.history);
  const restoreBin = useLiveQuery(() => db.restoreBin.toArray(), [], cachedState?.restoreBin);

  const initialStateToRender = useMemo<AppState | null>(() => {
    // Check if all live queries have returned data
    if (wallets && walletTypes && transactions && categories && tags && budgets && debts && scheduledTransactions && history && restoreBin) {
      return {
          wallets, walletTypes, transactions, categories, tags, budgets, debts, scheduledTransactions, history, restoreBin,
          settings: settings, // settings are now managed by SettingsProvider
      };
    }
    // If not all live queries are ready, return the cached state
    return cachedState;
  }, [wallets, walletTypes, transactions, categories, tags, budgets, debts, scheduledTransactions, history, restoreBin, cachedState, settings]);
