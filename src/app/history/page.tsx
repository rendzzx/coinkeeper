...
export default function HistoryPage() {
  ...
  const filteredLogs = historyLogs?.filter((log) => {
    ...
    const descriptionMatch = log.description
      ?.toLowerCase()
  ...
  const renderLogItem = (log: HistoryLog) => (
    <div
      key={log.id}
      className="flex flex-col sm:flex-row gap-4 p-4 border-b last:border-b-0 hover:bg-muted/50 cursor-pointer"
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-1">
          ...
          {log.context?.source === "system" && (
            <Badge
              variant="outline"
              className="flex items-center gap-1.5 cursor-help p-1 h-fit bg-muted/50"
            >
      ...
      {log.restoreId && log.status === "pending" && (
        ...
        >
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>{t("restoreItemQuestion")}</AlertDialogTitle>
                <AlertDialogDescription>
                  {t("restoreItemDescription")}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>{t("cancel")}</AlertDialogCancel>
                <AlertDialogAction
                  onClick={() => handleRestore(log.restoreId!)}
                >
                  {t("restore")}
                </AlertDialogAction>
              </AlertDialogFooter>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                <AlertDialogCancel>{t("cancel")}</AlertDialogCancel>
                <AlertDialogAction
                  onClick={() => handlePermanentDelete(log.restoreId!)}
          ...
          {filteredLogs && filteredLogs.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center text-center p-8 border-2 border-dashed rounded-lg">
              ...
              <h3 className="mt-4 text-xl font-semibold">
                {t("noHistoryLogsFound")}
              </h3>
              <p className="text-muted-foreground mt-2">
                {historyLogs && historyLogs.length > 0
                  ? t("searchReturnedNoResults")
                  : t("activitiesWillBeRecordedHere")}
              </p>
            </div>
          )}
        </CardContent>
